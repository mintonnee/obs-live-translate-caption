...
후속 계획: [005-caption-segments-and-versioning.md](005-caption-segments-and-versioning.md)가
아래 줄 혼합 창·초과 줄 절단·박스 크기 기반 번역 길이 힌트를 구간 버전·페이지 표시로
대체한다. 현재는 005 구현 미착수이며, 이 문서는 기존 구현의 기록이다. 폭 계산, 원문 tail과
설정 키 호환은 유지하고 구체적인 대체 성공 기준은 005 §4.7을 따른다.
...
| 시간 기반 페이지 넘김(긴 문장을 여러 화면에 순차 표시) | 002 구현에서는 잘라내기 + 프롬프트 길이 힌트로 대체한다. 후속 `005-caption-segments-and-versioning.md`에서 최소 표시 시간·만료·적체 상한과 함께 도입할 계획이다. |
