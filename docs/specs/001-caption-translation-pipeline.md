...
`gemini-3.5-flash-lite`에 번역을 요청한 뒤, 결과를 OBS 씬의 텍스트 소스에 자막 스타일로
...
| 번역 | final 전사 세그먼트마다 `gemini-3.5-flash-lite` generateContent를 호출해 대상 언어 텍스트를 얻는다. 직전 세그먼트를 문맥으로 함께 보낸다. |
...
6. `translate-protocol` 빌더/파서: 요청 JSON에 `system_instruction`, 마지막 `contents` 항목 `role: "user"`, `generationConfig.thinking_level: "minimal"`가 있고 `temperature`/`topP`/`topK`는 없다. 응답에서 `candidates[0].content.parts[*].text`를 이어 붙여 앞뒤 공백을 제거한 문자열을 반환하고, 후보가 없거나 `error`이면 실패를 반환한다(단위 테스트).
...
| 번역 모델 id는 `gemini-3.5-flash-lite`, generateContent REST: `POST https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent`, 헤더 `x-goog-api-key`. 출력은 텍스트 전용. | <https://ai.google.dev/gemini-api/docs/models/gemini-3.5-flash-lite> |
| Gemini 3.5 계열은 `generationConfig.thinking_level`(`minimal`/`low`/`medium`/`high`)을 지원하고, `temperature`/`top_p`/`top_k`는 요청에서 제거하라고 안내한다. 마지막 `contents` 항목은 `role: "user"`여야 한다. | <https://ai.google.dev/gemini-api/docs/whats-new-gemini-3.5> |
...
- 요청 빌더 `build_translate_request(target_code, target_name, context[], text)`:
  ...
  - `generationConfig`: `thinking_level: "minimal"`, `maxOutputTokens: 256`. 샘플링 파라미터 없음.
...
- `render(now_ms) -> optional<string>`: 최근 `max_segments`개 방출 세그먼트를 `\n`으로 이어
  붙인 문자열. 마지막 방출 시각 + `hold_ms`가 지나면 빈 문자열을 한 번 반환하고 이후는
...
|---|---|---|---|---|---|
| S0 스캐폴드 | 신규 파일 스텁 생성, 빌드 등록 | `CMakeLists.txt`, `tests/CMakeLists.txt`, 아래 S1–S5 파일의 빈 스텁 생성(이후 소유권은 각 슬라이스로 이전) | `src/*.cpp` 기존 파일 | 없음 | 미착수 |
| S1 caption-protocol | setup 빌더, 서버 메시지 파서 + 테스트 | `src/caption-protocol.hpp`, `src/caption-protocol.cpp`, `tests/test-caption-protocol.cpp` | 그 외 전부 | S0 | 미착수 |
| S2 translate-protocol | 요청 빌더, 응답 파서 + 테스트 | `src/translate-protocol.hpp`, `src/translate-protocol.cpp`, `tests/test-translate-protocol.cpp` | 그 외 전부 | S0 | 미착수 |
| S3 caption-composer | 순서 정렬/창/유지 시간 로직 + 테스트 | `src/caption-composer.hpp`, `src/caption-composer.cpp`, `tests/test-caption-composer.cpp` | 그 외 전부 | S0 | 미착수 |
| S4 caption-session | WebSocket 워커, HTTP 워커, 재연결, 계측 로그 | `src/caption-session.hpp`, `src/caption-session.cpp` | `src/translation-session.*`, `src/filter.cpp` | S1, S2, S3 | 미착수 |
| S5 OBS 통합 | 설정 키/UI, 모드 전환, 텍스트 소스 출력 | `src/filter.cpp`, `src/caption-output.hpp`, `src/caption-output.cpp`, `src/plugin-main.cpp` | `src/caption-session.*`, `src/source.cpp` | S4 | 미착수 |
| S6 문서 | README/AGENTS 갱신, 이 스펙 상태 갱신 | `README.md`, `AGENTS.md`, `docs/specs/*` | `src/`, `tests/` | S5 | 미착수 |
